...
const Contrato = ({goBack,cliente}) => {
  ...
  useEffect(()=>{if(canvasRef.current&&step==="assinar"){const ctx=canvasRef.current.getContext("2d");ctx.fillStyle="#0a0a14";ctx.fillRect(0,0,300,150);}});
  ...
  const clearC=()=>{const c=canvasRef.current,ctx=c.getContext("2d");ctx.fillStyle="#0a0a14";ctx.fillRect(0,0,c.width,c.height);setSigned(false);};

  const steps=["ver","fotos","assinar","ok"];
  const stepIdx=steps.indexOf(step);

  const Progress=()=>(
    <div style={{padding:"10px 20px 4px",background:"#0d0d1c"}}>
      <div style={{display:"flex",alignItems:"center"}}>
        {["Contrato","Fotos","Assinatura","Conclusão"].map((s,i)=>(
          <div key={i} style={{display:"flex",alignItems:"center",flex:1}}>
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
              <div style={{width:24,height:24,borderRadius:"50%",background:stepIdx>i?C.g:stepIdx===i?C.y:"rgba(255,255,255,0.1)",border:`2px solid ${stepIdx>i?C.g:stepIdx===i?C.y:"rgba(255,255,255,0.15)"}`,display:"flex",alignItems:"center",justifyContent:"center",color:stepIdx>i?"white":stepIdx===i?"#1a1000":C.m,fontSize:11,fontWeight:800}}>{stepIdx>i?"✓":i+1}</div>
              <span style={{fontSize:9,color:stepIdx===i?C.y:stepIdx>i?C.g:C.m,fontWeight:stepIdx>=i?700:400,whiteSpace:"nowrap"}}>{s}</span>
            </div>
            {i<3&&<div style={{flex:1,height:2,background:stepIdx>i?C.g:"rgba(255,255,255,0.08)",margin:"0 3px",marginBottom:14}}/>}
          </div>
        ))}
      </div>
    </div>
  ...
  if(step==="enviando")return(
    <div style={{padding:"40px 20px",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16,minHeight:300}}>
      <div style={{width:56,height:56,borderRadius:"50%",border:`3px solid rgba(255,255,255,0.1)`,borderTopColor:C.y,animation:"spin 0.8s linear infinite"}}/>
      ...
      <div style={{padding:"16px 16px 24px",display:"flex",flexDirection:"column",gap:14}}>
        ...
        {step==="ver"&&<>
          ...
          <div style={{background:"rgba(245,194,0,0.06)",border:"1px solid rgba(245,194,0,0.2)",borderRadius:16,padding:16,display:"flex",alignItems:"center",gap:14}}>
            ...
            <div><p style={{color:C.t,fontSize:15,fontWeight:700,margin:0}}>{cliente.nome}</p><p style={{color:C.s,fontSize:12,margin:"2px 0 0"}}>CPF: {maskCpf(cliente.cpf)}</p></div>
          </div>
          <div style={{background:"rgba(255,255,255,0.04)",border:`1px solid ${C.b}`,borderRadius:16,padding:16,display:"flex",flexDirection:"column",gap:0}}>
            {[["Contrato",`#${cliente.contratoId}`],["Plano",cliente.plano],["Valor",`R$ ${cliente.valor}/mês`],["Status",cliente.status||"Ativo"],["Vencimento","Todo dia 10"]].map(([k,v],i)=>(
              <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"10px 0",borderBottom:i<4?`1px solid rgba(255,255,255,0.06)`:undefined}}><span style={{color:C.s,fontSize:13}}>{k}</span><span style={{color:C.t,fontSize:13,fontWeight:700}}>{v}</span></div>
            ))}
          </div>
          <div style={{background:"rgba(255,255,255,0.03)",border:`1px solid ${C.b}`,borderRadius:14,height:160,overflowY:"auto",padding:14,fontSize:11,color:C.s,lineHeight:1.8}}>
            <p style={{color:C.t,fontWeight:700,fontSize:12,margin:"0 0 10px"}}>CONTRATO DE PRESTAÇÃO DE SERVIÇOS</p>
            <p><strong style={{color:"rgba(255,255,255,0.7)"}}>CONTRATADA:</strong> Oryx Internet Ltda.</p>
            <p><strong style={{color:"rgba(255,255,255,0.7)"}}>CONTRATANTE:</strong> {cliente.nome}</p>
            <p><strong style={{color:"rgba(255,255,255,0.7)"}}>CLÁUSULA 1:</strong> Prestação de serviços de internet, plano {cliente.plano}.</p>
            <p><strong style={{color:"rgba(255,255,255,0.7)"}}>CLÁUSULA 2:</strong> Valor R$ {cliente.valor}/mês, vencimento dia 10.</p>
            <p><strong style={{color:"rgba(255,255,255,0.7)"}}>CLÁUSULA 3:</strong> Vigência de 12 meses com renovação automática.</p>
        ...
        {step==="fotos"&&<>
          ...
          {[{k:"a",icon:"🪪",l:"Documento — Frente",s:"CNH ou RG (frente)"},{k:"b",icon:"🪪",l:"Documento — Verso",s:"CNH ou RG (verso)"},{k:"c",icon:"🤳",l:"Selfie com documento",s:"Segure o doc ao lado do rosto"}].map(f=>(
            <div key={f.k} style={{background:fotos[f.k]?"rgba(52,211,153,0.08)":"rgba(255,255,255,0.04)",border:`1px solid ${fotos[f.k]?"rgba(52,211,153,0.3)":C.b}`,borderRadius:16,padding:"14px 16px",display:"flex",alignItems:"center",gap:14}}>
              <div style={{width:44,height:44,borderRadius:12,background:fotos[f.k]?"rgba(52,211,153,0.15)":"rgba(255,255,255,0.06)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0}}>{fotos[f.k]?"✅":f.icon}</div>
        ...
        {step==="assinar"&&<>
          ...
          <div style={{background:"rgba(255,255,255,0.04)",border:`1px solid ${C.b}`,borderRadius:16,padding:12}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
              <span style={{color:C.s,fontSize:11,fontWeight:600,letterSpacing:1}}>ASSINATURA DO CLIENTE</span>
              <button onClick={clearC} style={{background:"none",border:`1px solid rgba(255,255,255,0.15)`,borderRadius:8,padding:"4px 10px",color:C.s,fontSize:11,cursor:"pointer"}}>Limpar</button>
            </div>
          ...
          <div style={{background:"rgba(255,255,255,0.03)",border:`1px solid rgba(255,255,255,0.06)`,borderRadius:12,padding:"12px 14px"}}><p style={{color:C.m,fontSize:11,margin:0,lineHeight:1.6}}>🔒 Registrado com IP, data, hora e CPF. Válido juridicamente (MP 2.200-2/2001).</p></div>
...
const MeusApps=({goBack})=>{
  ...
  if(done&&sel!==null){const a=APPS[sel];return(
    <div style={{flex:1,padding:"40px 20px",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16}}>
      <div style={{width:72,height:72,borderRadius:"50%",background:"rgba(52,211,153,0.15)",border:"2px solid rgba(52,211,153,0.4)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:32}}>✅</div>
      <h2 style={{color:C.t,fontSize:18,fontWeight:700,margin:0,textAlign:"center"}}>{a.ativo?"App desativado!":"App ativado!"}</h2>
      <p style={{color:C.s,fontSize:13,margin:0,textAlign:"center"}}>{a.nome} foi {a.ativo?"removido do":"adicionado ao"} seu plano.</p>
      <Btn label="Voltar para Meus Apps" onClick={()=>{setSel(null);setDone(false);}}/>
    </div>
  );}
  if(sel!==null){const a=APPS[sel];return(
    <div style={{padding:"20px 16px",display:"flex",flexDirection:"column",gap:14}}>
      <Back onClick={()=>setSel(null)}/>
      <div style={{display:"flex",alignItems:"center",gap:14}}>
        <div style={{width:56,height:56,borderRadius:16,background:a.cor,display:"flex",alignItems:"center",justifyContent:"center",fontSize:26,fontWeight:900,color:"white",flexShrink:0}}>{a.logo}</div>
        <div><h3 style={{color:C.t,fontSize:18,fontWeight:800,margin:0}}>{a.nome}</h3><p style={{color:C.s,fontSize:13,margin:"2px 0 0"}}>{a.desc}</p></div>
      </div>
      <div style={{background:"rgba(255,255,255,0.04)",border:`1px solid ${C.b}`,borderRadius:16,padding:16,display:"flex",flexDirection:"column",gap:0}}>
        {[["Status no plano",a.ativo?"Ativo":"Disponível"],["Valor adicional",a.ativo?"R$ 0,00/mês":"R$ 19,90/mês"],["Telas simultâneas","2 telas"],["Qualidade","Full HD"]].map(([k,v],i)=>(
          <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"10px 0",borderBottom:i<3?`1px solid rgba(255,255,255,0.06)`:undefined}}><span style={{color:C.s,fontSize:13}}>{k}</span><span style={{color:C.t,fontSize:13,fontWeight:700}}>{v}</span></div>
        ))}
      </div>
      <button onClick={()=>setDone(true)} style={{background:a.ativo?"rgba(241,85,85,0.1)":`linear-gradient(135deg,${a.cor},${a.cor}cc)`,color:a.ativo?C.r:"white",border:a.ativo?"1px solid rgba(241,85,85,0.3)":"none",borderRadius:14,padding:14,fontSize:15,fontWeight:800,cursor:"pointer"}}>
        {a.ativo?`Desativar ${a.nome}`:`Ativar ${a.nome}`}
      </button>
    </div>
  );}
  return(
    <div>
      <div style={{background:"linear-gradient(160deg,#151520,#1a1a2e)",padding:"18px 20px 22px"}}>
        <LogoH h={26}/>
        <h2 style={{color:C.t,fontSize:20,fontWeight:700,margin:"14px 0 4px"}}>Meus Apps</h2>
        <p style={{color:C.s,fontSize:13,margin:"0 0 14px"}}>Gerencie seus streamings em um só lugar</p>
        <div style={{display:"flex",gap:10}}>
          {[["Ativos",APPS.filter(a=>a.ativo).length,C.g],["Disponíveis",APPS.filter(a=>!a.ativo).length,C.y],["Total",APPS.length,C.s]].map(([l,v,c])=>(
            <div key={l} style={{flex:1,background:"rgba(255,255,255,0.05)",borderRadius:12,padding:"10px",textAlign:"center"}}><p style={{color:c,fontSize:20,fontWeight:800,margin:0}}>{v}</p><p style={{color:C.s,fontSize:11,margin:0}}>{l}</p></div>
          ))}
        </div>
      </div>
      <div style={{padding:"16px 16px 24px",display:"flex",flexDirection:"column",gap:10}}>
        {APPS.map((a,i)=>(
          <div key={i} onClick={()=>setSel(i)} style={{background:a.bg,border:`1px solid ${a.cor}22`,borderRadius:18,padding:16,display:"flex",alignItems:"center",gap:14,cursor:"pointer"}}>
            <div style={{width:52,height:52,borderRadius:14,background:a.cor,display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,fontWeight:900,color:"white",flexShrink:0}}>{a.logo}</div>
            <div style={{flex:1}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:3}}><p style={{color:C.t,fontSize:15,fontWeight:700,margin:0}}>{a.nome}</p>{a.ativo&&<span style={{background:"rgba(52,211,153,0.15)",border:"1px solid rgba(52,211,153,0.3)",borderRadius:10,padding:"1px 7px",color:C.g,fontSize:10,fontWeight:700}}>ATIVO</span>}</div><p style={{color:C.s,fontSize:12,margin:0}}>{a.desc}</p></div>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={C.s} strokeWidth="2"><polyline points="9 18 15 12 9 6"/></svg>
          </div>
        ))}
      </div>
    </div>
