...
const Contrato = ({goBack,cliente}) => {
  ...
  return (
    <div style={{padding:"16px 16px 28px",display:"flex",flexDirection:"column",gap:16}}>
      <Back onClick={goBack}/>
      <div><h2 style={{color:C.t,fontSize:20,fontWeight:800,margin:0}}>Meu Contrato</h2><p style={{color:C.s,fontSize:13,margin:"4px 0 0"}}>Termo de adesão e dados do seu contrato</p></div>

      <div style={{background:assinado?"rgba(52,211,153,0.08)":"rgba(245,194,0,0.08)",border:`1px solid ${assinado?"rgba(52,211,153,0.3)":"rgba(245,194,0,0.3)"}`,borderRadius:16,padding:16,display:"flex",alignItems:"center",gap:14}}>
        <div style={{width:46,height:46,borderRadius:"50%",background:assinado?"rgba(52,211,153,0.15)":"rgba(245,194,0,0.15)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0}}>{assinado?"✅":"✍️"}</div>
        <div style={{flex:1}}>
          <p style={{color:C.t,fontSize:15,fontWeight:700,margin:"0 0 2px"}}>{assinado?"Contrato assinado":"Termo pendente de assinatura"}</p>
          <p style={{color:C.s,fontSize:12,margin:0}}>{assinado?"Seu termo de adesão já foi assinado.":"Você ainda precisa assinar seu termo de adesão."}</p>
        </div>
      </div>

      <div style={{background:C.surf,border:`1px solid ${C.b}`,borderRadius:16,padding:16,display:"flex",flexDirection:"column",gap:0}}>
        {[["Cliente",cliente.nome],["Contrato",`#${cliente.contratoId}`],["Plano",cliente.plano],["Status",cliente.status||"Ativo"],["Vencimento",cliente.vencimento?`Todo dia ${cliente.vencimento}`:"—"]].map(([k,v],i,a)=>(
          <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"10px 0",borderBottom:i<a.length-1?`1px solid ${C.line}`:undefined}}><span style={{color:C.s,fontSize:13}}>{k}</span><span style={{color:C.t,fontSize:13,fontWeight:700,textAlign:"right",maxWidth:"60%"}}>{v}</span></div>
        ))}
      </div>

      {!assinado&&(url
        ? <Btn label="Assinar contrato →" onClick={()=>abrir(url)}/>
        : <div style={{background:C.surf,border:`1px solid ${C.line}`,borderRadius:12,padding:"14px 16px"}}><p style={{color:C.s,fontSize:12,margin:0,lineHeight:1.6}}>O link de assinatura ainda não está disponível para este contrato. Fale com o suporte para gerar seu termo de adesão.</p></div>
      )}
      {pdf&&<Btn label="Baixar termo (PDF)" onClick={()=>abrir(pdf)} s={{background:C.surf2,color:C.t,boxShadow:"none",border:`1px solid ${C.line2}`}}/>}

      <div style={{background:C.surf,border:`1px solid ${C.line}`,borderRadius:12,padding:"12px 14px"}}><p style={{color:C.m,fontSize:11,margin:0,lineHeight:1.6}}>🔒 A assinatura é feita na plataforma oficial SGPsign, com validade jurídica (MP 2.200-2/2001).</p></div>
    </div>
  );
};
...
const Velocidade=({goBack})=>{
  ...
  return (
    <div style={{padding:"16px 16px 28px",display:"flex",flexDirection:"column",gap:16}}>
      ...
      <div style={{background:C.surf,border:`1px solid ${C.b}`,borderRadius:18,padding:"18px",display:"flex",flexDirection:"column",gap:6}}>
        <p style={{color:C.t,fontSize:14,fontWeight:700,margin:0}}>Teste completo</p>
        <p style={{color:C.s,fontSize:12,margin:0,lineHeight:1.5}}>Mede download, upload e latência (ping) com detalhes, pelo Speedtest.</p>
        <button onClick={()=>abrir("https://www.speedtest.net/pt")} style={{marginTop:8,width:"100%",background:C.surf2,color:C.t,border:`1px solid ${C.line2}`,borderRadius:12,padding:13,fontSize:14,fontWeight:600,cursor:"pointer"}}>Abrir teste completo</button>
      </div>
...
// ─── TAB BAR ───
const TABS=["home","boleto","velocidade","apps","os"];
const TLABELS={home:"Início",boleto:"Boleto",velocidade:"Velocidade",apps:"Meus Apps",os:"Chamados",suporte:"Suporte"};
const TICONS={home:Ico.home,boleto:Ico.boleto,velocidade:Ico.velocidade,apps:Ico.apps,os:Ico.os,suporte:Ico.suporte};
