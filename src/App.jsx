  ...
  return (
    <div style={{padding:"32px 22px 28px",display:"flex",flexDirection:"column",gap:20,position:"relative"}}>
      ...
      <div style={{background:C.surf,border:`1px solid ${C.b}`,borderRadius:20,padding:"22px 18px",display:"flex",flexDirection:"column",gap:14,position:"relative"}}>
        ...
        <button onClick={()=>onAuth({...DEMO_CONTA,demo:true})} style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,background:"rgba(37,211,102,0.07)",border:"1.5px solid rgba(37,211,102,0.2)",borderRadius:14,padding:13,color:C.wa,fontSize:14,fontWeight:600,cursor:"pointer"}}>
...
export default function App(){
  ...
  const onAuth=(a)=>{ setConta(a); if(a.contratos.length===1){ setCliente(mkCliente(a,a.contratos[0])); setScreen("main"); } else { setScreen("selecao"); } };
  const escolher=(ct)=>{ setCliente(mkCliente(conta,ct)); setScreen("main"); };
  ...
  const goLogin=()=>{setScreen("login");setTab("home");setCliente(null);setConta(null);};
